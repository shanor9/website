/* Generated file: js/app.direct.js
 * Source entry: js/app.js
 * Do not edit manually.
 */
(() => {
  // js/application/snake/SnakeSimulation.js
  ...
  var SnakeSimulation = class {
    ...
    step(now) {
      ...
      let steer = this.turnDrift + randomRange(-4e-3, 4e-3);
      ...
      if (pointerIsActive) {
        ...
        if (distance < this.fleeRadius) {
          ...
          const awayAngle = distance > 1e-4 ? Math.atan2(fromPointerY, fromPointerX) : this.head.angle + Math.PI;
      ...
      if (this.head.x < this.edge || this.head.x > this.width - this.edge || this.head.y < this.edge || this.head.y > this.height - this.edge) {
  ...
  // js/infrastructure/browser/motion.js
  function prefersReducedMotion() {
    return window.matchMedia("(prefers-reduced-motion: reduce)").matches;
  }

  // js/application/grenade/GrenadeArc.js
  var GrenadeArc = class {
  ...
  // js/presentation/hero/GrenadeLauncherController.js
  function randomRange2(min, max) {
    return min + Math.random() * (max - min);
  }
  var PROJECTILE_WIDTH = 24;
  var PROJECTILE_HEIGHT = PROJECTILE_WIDTH * 36 / 64;
  var GrenadeLauncherController = class {
    ...
    handleShoot() {
      const originRect = this.muzzleAnchor ? this.muzzleAnchor.getBoundingClientRect() : this.button.getBoundingClientRect();
      ...
      const projectile = new GrenadeArc({
        x: launchX,
        y: launchY,
        vx: randomRange2(330, 415),
        vy: randomRange2(-150, -108),
        gravity: randomRange2(430, 520),
        lifeMs: randomRange2(1300, 1800)
      });
    ...
    loop(timestamp) {
      ...
      this.projectiles = this.projectiles.filter((entry) => {
        ...
        entry.node.style.transform = `translate(${x - PROJECTILE_WIDTH / 2}px, ${y - PROJECTILE_HEIGHT / 2}px) rotate(${angle}deg)`;
  ...
  // js/presentation/snake/SnakeCanvasController.js
  var SnakeCanvasController = class {
  ...
  function bootstrapGrenadeLauncher() {
    ...
    const muzzleAnchor = (_a = button == null ? void 0 : button.querySelector(".tee-muzzle-anchor")) != null ? _a : null;
